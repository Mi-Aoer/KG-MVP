...
下面这份“开发任务拆解书”按当前已收口的一期范围编写，目标只有一个：**让开发人员按最少功能、最少耦合、最少代码量，顺序完成“图谱构建闭环”**。
...
1. **后端先行，前端后接。**
   先用 Swagger 把当前一期的图谱构建链路跑通，再写前端页面。
2. **先接 mock，再接真实模型。**
   当前版本只要求**抽取模型**先做 mock 适配，避免开发被外部 API 阻塞。
3. **不引入多余中间层。**
   这版采用：
...
5. **所有模型调用都走统一接口，但允许 provider / 模型适配。**
   当前版本只有抽取链路复用 `llm_client` 抽象；不得假设所有 OpenAI-compatible provider 共享同一消息结构、同一 SDK 行为和同一传输策略。
6. **图谱变更后的规则要简单。**
   只要三元组或 schema 被改过，就把项目状态从 `imported` 打回 `initialized`。
   这意味着：**图谱已过期，后续应优先用 `rebuild` 重建，而不是指望增量同步删除。**

> 额外口径：问答链路已移出当前一期范围。即使历史版本中存在 `qa_config_id`、`qa_logs` 或问答章节，也统一按“预留、不启用、不纳入本期验收”处理。
...
> `T10` 原问答链路已移出当前一期范围，保留编号仅用于与旧版文档对照。
...
把当前一期图谱构建闭环所需的核心表一次性定好，后续功能都基于这些表展开。
...
class ModelConfig(Base):
    ...
    config_type: Mapped[str] = mapped_column(String(20), nullable=False)   # 当前版本仅启用 extract
...
class SourceRecord(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=uuid_str)
    project_id: Mapped[str] = mapped_column(String, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False)
    batch_id: Mapped[str] = mapped_column(String, ForeignKey("import_batches.id", ondelete="CASCADE"), nullable=False)
...
class ExtractedTriple(Base):
    ...
    project_id: Mapped[str] = mapped_column(String, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False)
    batch_id: Mapped[str] = mapped_column(String, ForeignKey("import_batches.id", ondelete="CASCADE"), nullable=False)
    source_id: Mapped[str] = mapped_column(String, ForeignKey("source_records.id", ondelete="CASCADE"), nullable=False)
...
说明：上面的代码块只给开发起步骨架；各表的唯一约束、索引和级联策略，以《系统设计文档_修订版_一期范围收缩版》中的当前表结构清单为准。

------

## 3. 预留结构处理规则

如果当前仓库或旧版本表结构里已经存在以下内容：

- `projects.qa_config_id`
- `model_configs` 中的 `qa` 类型配置
- `qa_logs`

可以暂时保留为**预留结构**，但必须同时满足：

1. 当前版本不在 DTO、接口、service 中启用；
2. 当前版本不把它们作为项目创建、抽取、导入的前置依赖；
3. 当前版本不纳入验收清单；
4. 文档中明确写成“预留，不启用”。
...
- SQLite 文件生成成功
- 当前版本 8 张业务表 + 1 张错误表存在
- 重新启动后不会重复报错
- 外键可正常级联
- 预留的问答结构若物理存在，也不会被当前链路依赖
...
5. 如库中还保留 `qa_logs` 等历史表，只核对“未被当前接口依赖”，不作为本期验收项。
...
先把“项目”和“抽取模型配置”建起来，后面所有抽取、schema 和导入流程都依赖它们。
...
- 项目创建时只校验 `extract_config_id` 是否存在且类型正确；
- 项目创建成功后状态直接置为 `ready`；
- 如数据库里仍保留 `qa_config_id` 等预留字段，service 不应读写它们。
...
- 删除未被项目引用的配置可成功
- 删除项目可成功
- 当前版本项目创建和运行流程不依赖任何问答配置

## 验收流程

1. 通过 Swagger 建一条 `extract` 配置。
2. 用这条配置建项目。
3. 查询项目列表。
4. 修改项目说明。
5. 删除项目。
...
def build_provider_options(base_url: str, model_name: str, instruction: str) -> dict[str, Any]:
    ...
    if any(key in instruction_lower for key in ["json", "三元组", "schema"]):
...
- 当前版本不要在 `llm_client` 中继续保留 Cypher 生成或答案生成分支
...
# T10 后续阶段预留：图谱问答（暂缓开发，不纳入本期）

## 说明

原来的 `T10 问答链路：Cypher 生成、校验、执行、答案生成` 已移出当前一期范围。
保留 `T10` 编号仅用于与旧版任务书、旧版进度记录和历史讨论对照。

## 当前处理原则

1. 不实现问答接口、DTO、service、路由和前端页面。
2. 不继续编写 Cypher 生成提示词、Cypher 校验器和问答日志落库逻辑。
3. 不将 `qa_service.py`、`cypher_validator.py`、`qa.py`、`QaPage`、`qa.ts` 等内容纳入当前开发顺序。
4. 若历史版本已经存在相关空壳文件或预留表结构，只保留为预留，不启用、不纳入验收。
5. 待后续问答方案明确后，再单独补一版需求、设计、任务与验收文档。
...
在后端 Swagger 的图谱构建链路可跑通后，再上前端，把 3 个必要页面接起来。
...
### C. Schema / Graph 测试 `schema_service` + `graph_service`
...
2. 新建项目
3. 上传 txt
4. 先用 mock 发起抽取并看进度直到完成
5. 再用真实 provider + 实际抽取提示词完成一次兼容性验收
6. 修改一条 raw_response 并 reparse
7. 手工新增一条三元组
8. refresh schema
9. 新增一个实体类型
10. init graph
11. import graph
12. 去 Neo4j Browser 抽查数据
13. rebuild graph
14. 删除项目
15. 确认 SQLite 和 Neo4j 中项目数据都消失
...
由未参与开发的人，按 README 从零启动一遍，若能成功跑通“建项目 -> 抽取 -> 修正 -> schema -> 导入”整条链路，即视为通过。
...
如果代码仓库里仍残留 `qa_*` 文件、`qa_logs` 表或问答配置字段：

- 当前版本不继续扩展；
- 当前版本不注册接口；
- 当前版本不纳入联调和验收；
- 后续是否删除或复用，等问答方案明确后再定。

------

## 7. 每次验收通过后都要提交 Git

从当前版本起，每完成一个新的可独立验收进展，统一按以下方式执行：

- 先按对应任务的“验收流程”完成本地验证；
- 验收通过后，立即将本次通过验收的代码执行 `git add` 和 `git commit`；
- `git commit` 必须附带简短提交描述，说明本次进展对应的任务或结果；
- 未验收通过的中间状态，不作为正式进展提交；
- 提交完成后，再同步更新 `docs/项目开发进度记录.md`。

推荐提交描述风格：

- `feat: complete T08 schema maintenance`
- `feat: implement T09 graph import`
- `fix: repair T07 reparse flow`
...
`T10` 作为后续阶段预留，不纳入当前最短落地路径。

不要并行做太多。
**最省时间的方式就是：每完成一个任务，就立刻做该任务的验收流程；确认通过后马上提交 Git，再进入下一步。**
